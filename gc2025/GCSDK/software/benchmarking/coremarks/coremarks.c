...
#error "Please define a way to initialize a memory block."
	...
	for (i=0 ; i<MULTITHREAD; i++) {
		...
		}//printf("Corelist mat int\n");
		if (results[i].execs & ID_STATE) {
			core_init_state(results[0].size,results[i].seed1,results[i].memblock[3]); //stuck
		//printf("Corelist state int\n");
		}
	...
	if (time_in_secs(total_time) > 0)
		ee_printf("Iterations/Sec   : %lu\n",(long unsigned) default_num_contexts*results[0].iterations/time_in_secs(total_time)); //Changed
		//ee_printf("Iterations/Sec   : %d\n",default_num_contexts*results[0].iterations/time_in_secs(total_time)); Unchanged
#endif
	...
	printf("seedcrc          : 0x%x\n",seedcrc);
		...
		for (i=0 ; i<default_num_contexts; i++) 
			printf("[%d]crclist       : 0x%x\n",i,results[i].crclist);
	if (results[0].execs & ID_MATRIX) 
		for (i=0 ; i<default_num_contexts; i++) 
			printf("[%d]crcmatrix     : 0x%x\n",i,results[i].crcmatrix);
	if (results[0].execs & ID_STATE)
		for (i=0 ; i<default_num_contexts; i++) 
			printf("[%d]crcstate      : 0x%x\n",i,results[i].crcstate);
	for (i=0 ; i<default_num_contexts; i++) 
		printf("[%d]crcfinal      : 0x%x\n",i,results[i].crc);
	if (total_errors==0) {
