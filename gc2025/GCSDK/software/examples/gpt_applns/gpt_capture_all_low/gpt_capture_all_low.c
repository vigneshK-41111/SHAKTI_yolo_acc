 ...
 * Brief Description of file            :  To check the functionality of GPT capture mode when set to active low.
...
int main()
{
	...
	printf("\n UP COUNTER OVERFLOW capture low TEST CODE");
...
#endif	
{
		instance = gpt_configure_ctrl_reg(module, up_cntr, over_flow, 10000, 0x0); //clk_src = 0 internal clock is used
		...
		instance->gpt_count_reference = 1000000; //Disable reset counter
		printf("\n instance[%x]->gpt_control1: %x",  module, instance->gpt_control);
		instance->gpt_control = 0x0075;
		printf("\n instance[%x]->gpt_control1: %x",  module, instance->gpt_control);
		gpt_start(instance);
}
	module = MODULE_NO;
