 ...
 * Brief Description of file            :  verify the up down counter mode of gpt
...
uint8_t upcntr_sts[4] = {0}, dwncntr_sts[4] = {0};

/** @fn gpt_timer_interrupt() 
 * @brief gpt timer interrupt for all gpt up-down counters
 * @details Gets the gpt instance, checks for the overflow & underflow interrupt, clear the count value and set the status variable.
 * @warning No warning
...
unsigned gpt_timer_interrupt (unsigned num)
{
	...
	switch(num)
	{
		case 39:
		   instance = gpt_instance[0];
#if 1
			if ( status1 & 0x4000 )//down counter
			{
				status_new |= 0x4000;
				if(status1 & 0x400)
				{
...
#endif
#if 0
			else if ( status1 & 0x2000 )//up counter
			{
				status_new |= 0x2000;
				if(status1 & 0x200)
				{
...
int main()
{
	...
	gpt_init();
	// plic_init();
	log_info("\n\n\n ****  UP DOWN  COUNTER OVERFLOW UNDERFLOW TEST CODE  **** \n\n\n");
...
#endif
	{
		instance = gpt_configure_ctrl_reg(module, updown_cntr, under_flow, 10000, 0x0); //clk_src = 0 internal clock is used
