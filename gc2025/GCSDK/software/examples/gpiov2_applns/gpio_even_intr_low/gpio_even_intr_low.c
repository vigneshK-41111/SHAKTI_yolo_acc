/***************************************************************************
 * Project           			:  GPIOV2
 * Name of the file	     		:  gpio_even_intr_low.c
 * Created date			        :  08.01.2024
 * Brief Description of file    :  Interrupt test for GPIO active  low even port interrupts.
...
/***************************************************************************
 * Project           			:  GPIOV2
 * Name of the file	     		:  gpio_odd_intr_high.c
 * Created date			        :  08.01.2024
 * Brief Description of file    :  Interrupt test for GPIO active  high odd port interrupts.
...
unsigned handle_button_press(unsigned int_id)
{
    ...
    printf("\n even GPIO %d interrupt occured", gpio_num);
    gpiov2_instance->set = (1 << (gpio_num + 1));  	 // clear the pin that triggered the interrupt by setting it high
    return 0;
}
/**
...
int main(void)
{
	...
	gpiov2_init();
	plic_init();
	printf("\n GPIO Active low even Interrupt Test code");
	gpiov2_instance->direction= GPIO_OUTPUTS; // GPIO0 = output, GPIO 1=input interrupt
	printf("\n Direction reg: %x", gpiov2_instance->direction);
	gpiov2_instance->set = GPIO_OUTPUTS; // Make gpio0 (output) as low by writing 1 to clear register
	printf("\n After setting GPIOs %x \n", gpiov2_instance->data);
	gpiov2_instance->intr_config = 0xffff;		//By default, the interrupts are active high, if active low interrupts are required that particular GPIO bit in GPIOINTCONFIG register should be set.
	// assign ISRs only for input pins 
