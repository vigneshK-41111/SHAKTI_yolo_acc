...
unsigned handle_button_press(unsigned int_id)
{
    ...
    printf("\n odd GPIO %d active high interrupt occurred!", gpio_num);
    gpiov2_instance->clear = (1 << (gpio_num - 1));   // clear the pin that triggered
    return 0;
}
/**
...
int main(void)
{
	gpiov2_init();
	plic_init();
	printf("\n GPIO Active High Odd Interrupt Test code");
