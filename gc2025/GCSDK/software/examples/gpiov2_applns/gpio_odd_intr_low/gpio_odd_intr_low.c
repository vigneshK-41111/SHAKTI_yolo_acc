...
#define  INTERRUPT_1 (GPIO1 | GPIO3 | GPIO5 | GPIO7 | GPIO9 | GPIO11 | GPIO13 | GPIO15 )
//EVEN PINS OUTPUT 
#define GPIO_OUTPUTS (GPIO0 | GPIO2 | GPIO4 | GPIO6 | GPIO8 | GPIO10 | GPIO12 | GPIO14 )
...
unsigned handle_button_press(unsigned int_id)
{
    ...
    printf("\n odd GPIO %d active low interrupt occurred", gpio_num);
    gpiov2_instance->set = (1 << (gpio_num - 1));  	 // clear the pin that triggered
    return 0;
}
/**
...
int main(void)
{
	...
	gpiov2_init();
	plic_init();
	printf("\n GPIO Active low Odd Interrupt Test code");
