 ...
 * Name of the file	            	    :  sspi_MSB_check.c
 * Brief Description of file            :  sspi MSB check example code.
...
/**
 * @brief Main function to test SSPI1 in MSB-first full-duplex communication mode.
 * This function performs the following:
 * - Configures pinmux and UART (if running on BSOC). Initializes the SSPI module.
 * - Configures SSPI with clock, phase, polarity, setup/hold time, and MSB-first format.
 * - Transmits a series of bytes via SSPI.Receives and verifies the transmitted data if available.
 * This loop continuously fills TX FIFO with incrementing values, waits for transfer completion,
 * and prints received data( in MSB first format) from the RX FIFO.
