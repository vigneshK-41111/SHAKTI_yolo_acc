...
/**
 * @brief  SSPI0 RX Quad Interrupt Service Routine.
 * This ISR handles the SPI0 Receive Quad interrupt. When the RX Quad flag is set
 * in the FIFO status register, it reads `SSPI_SIZE` bytes from the SSPI data RX register
 * into the `spi1_rx[]` buffer. It also sets a flag `spi0_intr_flag` indicating the interrupt occurred.
...
unsigned spi0_rx_interrupt(unsigned num)
{
	...
	if (sspi_instance[SSPI_INSTANCE]->fifo_status & SPI_RX_QUAD)
	{
		log_info("\n SPI 0 Receive Quad interrupt occurred");
...
/**
 * @brief  SSPI1 RX Quad Interrupt Service Routine.
 * This ISR handles the SPI1 Receive Quad interrupt. When the RX Quad flag is set
 * in the FIFO status register, it reads `SSPI_SIZE` bytes from the SSPI data RX register
 * into the `spi1_rx[]` buffer. It also sets a flag `spi1_intr_flag` indicating the interrupt occurred.
 * @param  num  The interrupt number 
 * @return Always returns 0 
 */
...
unsigned spi1_rx_interrupt(unsigned num)
{
	...
	if (sspi_instance[SSPI_INSTANCE]->fifo_status & SPI_RX_QUAD)
	{
		...
		log_info("\n SPI 1 Receive Quad interrupt occured");
	}
	spi1_intr_flag = 1;
	log_debug("\n sspi_instance[SSPI_INSTANCE]->fifo_status: %x", sspi_instance[SSPI_INSTANCE]->fifo_status);
	log_debug("\n sspi_instance[SSPI_INSTANCE]->comm_status: %x", sspi_instance[SSPI_INSTANCE]->comm_status);
...
/** @fn main()
 * @brief main function that  Demonstrates SSPI full-duplex communication with RX Quad interrupt on SSPI1.
 * @details This function configures SSPI1 in full-duplex mode, enables RX Quad interrupt, and fills TX FIFO.
 * Upon reception, an interrupt service routine (ISR) handles the received data.
 */
...
int main()
{
	...
	*(pinmux_config_reg) = 0x154000;
	...
	sspi_instance[SSPI_INSTANCE]->intr_en = SPI_RX_QUAD_INTR_EN;
