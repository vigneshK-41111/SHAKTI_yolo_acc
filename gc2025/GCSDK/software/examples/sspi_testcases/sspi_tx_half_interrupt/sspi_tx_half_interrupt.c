...
/**
 * @fn unsigned spi0_tx_half_interrupt(unsigned num)
 * @brief SPI0 Transmit Half-FIFO interrupt handler.
 * @details This ISR handles the SPI0 half-full TX FIFO condition. It checks the `SPI_TX_HALF` flag in the FIFO status register,logs the interrupt event.Then, it sends a dummy byte `0x0A` and sets the `spi0_intr_flag`.
 * @param num Unused interrupt number parameter.
 * @return Always returns 0.
 */
unsigned spi0_tx_half_interrupt (unsigned num)
{
...
if(sspi_instance[SSPI_INSTANCE]->fifo_status & SPI_TX_HALF)
	{
		log_info("\n SPI 0 Transmit Half interrupt occurred");
	}
...
/**
 * @fn unsigned spi1_tx_half_interrupt(unsigned num)
 * @brief SPI1 Transmit Half-FIFO interrupt handler.
 * @detailsThis ISR handles the SPI1 half-full TX FIFO condition. It checks the `SPI_TX_HALF` flag in the FIFO status register and logs the    interrupt event.Then, it sends a dummy byte `0x0A` and sets the `spi1_intr_flag`.
 * @param num Unused interrupt number parameter.
 * @return Always returns 0.
 */
unsigned spi1_tx_half_interrupt (unsigned num)
{
	...
	if(sspi_instance[SSPI_INSTANCE]->fifo_status & SPI_TX_HALF)
	{
		log_info("\n SPI 1 Transmit Half interrupt occurred ");
	}
	sspi_instance[SSPI_INSTANCE]->data_tx = 0x0a;
	spi1_intr_flag = 1;
	log_debug("\n sspi_instance[SSPI_INSTANCE]->fifo_status: %x", sspi_instance[SSPI_INSTANCE]->fifo_status);
...
/**
 * @file sspi_tx_half_interrupt.c
 * @brief Example code to demonstrate SSPI transmit half interrupt functionality
 * @details This code initializes the SSPI peripheral, configures it in master mode,
 * enables transmit-half interrupts, and sends data( 16 bytte)  to the transmit FIFO.
 * @return Returns 0 (never reached as code loops indefinitely)
 */
...
int main()
{
	...
	*(pinmux_config_reg) = 0x154000;
	...
	isr_table[PLIC_INTERRUPT_35] = spi1_tx_half_interrupt;
	...
	sspi_instance[SSPI_INSTANCE]->intr_en = SPI_TX_HALF_INTR_EN;
	while(1);	
	return 0;
}
