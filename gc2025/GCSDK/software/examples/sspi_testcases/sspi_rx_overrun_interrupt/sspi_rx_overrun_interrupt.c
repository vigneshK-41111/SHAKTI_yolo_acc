 ...
 * @details interrupt service routine (ISR) is triggered by SPI0 RX events.
 * It specifically checks for a **receive overrun** condition using the `comm_status` register.
 * Checks if the `SPI_OVR` (overrun) bit is set in `comm_status`. If set, it logs that an RX overrun interrupt occurred.
 * - Sets a software flag `spi0_intr_flag` to notify the main loop.
...
if(sspi_instance[SSPI_INSTANCE]->comm_status & SPI_OVR)
	{
		log_info("\n SPI 0 Receive overrun interrupt occured");
	}
	spi0_intr_flag = 1;
 ...
 * @details interrupt service routine (ISR) is triggered by SPI1 RX events.
 * It specifically checks for a **receive overrun** condition using the `comm_status` register.
 * Checks if the `SPI_OVR` (overrun) bit is set in `comm_status`. If set, it logs that an RX overrun interrupt occurred.
 * - Sets a software flag `spi1_intr_flag` to notify the main loop.
 * @param num Interrupt number .
 * @return 0
 */
unsigned spi1_rx_interrupt (unsigned num)
{
	...
	if(sspi_instance[SSPI_INSTANCE]->comm_status & SPI_OVR)
	{
		log_info("\n SPI 1 Receive overrun interrupt occured");
			for(int i = 0; i < 32; i++)
				{
...
/**
 * @brief SPI2 RX Interrupt Handler.
 * @details interrupt service routine (ISR) is triggered by SPI2 RX events.
 * It specifically checks for a **receive overrun** condition using the `comm_status` register.
 * Checks if the `SPI_OVR` (overrun) bit is set in `comm_status`. If set, it logs that an RX overrun interrupt occurred.
 * - Sets a software flag `spi3_intr_flag` to notify the main loop.
 * @param num Interrupt number .
 * @return 0
 */
...
/**
 * @brief Entry point for SSPI1 RX Overrun Interrupt test.
 * @details This function sets up and tests the SSPI1 RX overrun interrupt in a full-duplex SPI configuration.
 * - Initializes the SSPI peripheral and enables its clock.
 * - Clears the SPI FIFOs and waits for the peripheral to become idle.
 * - Configures SPI master settings such as clock rate, polarity, phase, setup/hold time, etc.
 * - Populates transmit (`spi1_tx[]`) and receive (`spi1_rx[]`) buffers.
 * - First, transmits a block of data to test RX functionality.Then re-transmits data again to intentionally generate a **receive overrun**.
 * - Enables RX overrun interrupt (`SPI_RX_OVERRUN_INTR_EN`) to check if the overrun is correctly detected.
 * @return Always returns 0 (though never actually reached due to infinite loop).
 */
...
int main()
{
	...
	*(pinmux_config_reg) = 0x154000;
	...
	sspi_instance[SSPI_INSTANCE]->intr_en = SPI_RX_OVERRUN_INTR_EN;
	sspi_configure_tx_rx_length(sspi_instance[SSPI_INSTANCE], SSPI_SIZE * DATA_SIZE, SSPI_SIZE * DATA_SIZE);
	sspi_configure_comm_mode(sspi_instance[SSPI_INSTANCE], FULL_DUPLEX);
	sspi_enable_txrx(sspi_instance[SSPI_INSTANCE], ENABLE);
