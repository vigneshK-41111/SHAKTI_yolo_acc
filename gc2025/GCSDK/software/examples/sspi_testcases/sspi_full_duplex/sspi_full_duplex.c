...
/**
 * @brief Main function to test SSPI1 full-duplex communication.
 * @details This function Initializes UART and SSPI based on platform configuration.
 * - Configures SSPI1 with fixed clock, polarity, and phase.Sends incremental test data over SSPI in full-duplex mode.
 * - Reads back the data and prints it. Runs continuously in a loop.
 * @note Loopback connection (MOSI ↔ MISO) is required for correct verification.
 * @return int Always returns 0. Runs indefinitely for repeated checks.
 */
