...
/**
 * @brief SPI0 RX Half-Full Interrupt Handler.
 * @details interrupt service routine (ISR) is triggered when the SPI0 RX FIFO reaches half capacity(15 bytes).
 * It processes the incoming data and stores it in the `spi0_rx` buffer.
 * - Sets a software flag `spi0_intr_flag` to indicate the ISR was triggered.
 * @param num Interrupt number assigned by PLIC (not functionally used in body).
 * @return Interrupt number (optional for interrupt management tracking).
 */
...
if(sspi_instance[SSPI_INSTANCE]->fifo_status & SPI_RX_HALF)
	{
		for(int i = 0; i < SSPI_SIZE; i++)
		{
			spi1_rx[i] =  sspi_instance[SSPI_INSTANCE]->data_rx;				
		}
		log_info("\n SPI 0 Receive Half interrupt occured");
	}
	spi0_intr_flag = 1;
...
/**
 * @brief SPI1 RX Half-Full Interrupt Handler.
 * @details interrupt service routine (ISR) is triggered when the SPI1 RX FIFO reaches half capacity(15 bytes).
 * It processes the incoming data and stores it in the `spi1_rx` buffer.
 * - Sets a software flag `spi1_intr_flag` to indicate the ISR was triggered.
 * @param num Interrupt number assigned by PLIC (not functionally used in body).
 * @return Interrupt number (optional for interrupt management tracking).
 */
unsigned spi1_rx_interrupt (unsigned num)
{
	...
	if(sspi_instance[SSPI_INSTANCE]->fifo_status & SPI_RX_HALF)
	{
		...
		log_info("\n SPI 1 Receive Half interrupt occured");
	}
	spi1_intr_flag = 1;
	log_debug("\n sspi_instance[SSPI_INSTANCE]->fifo_status: %x", sspi_instance[SSPI_INSTANCE]->fifo_status);
	log_debug("\n sspi_instance[SSPI_INSTANCE]->comm_status: %x", sspi_instance[SSPI_INSTANCE]->comm_status);
...
/**
 * @brief Main function to test SSPI1 Receive HalfInterrupt in Full-Duplex Mode.
 * @details function initializes the SSPI peripheral in master mode and enables the RX Half-Full interrupt.
 * It sets up the SPI transmission and waits for interrupts, validating that receive interrupts are triggered
 * when the RX FIFO reaches the half-full threshold.
...
int main()
{
	...
	*(pinmux_config_reg) = 0x154000;
	...
	sspi_instance[SSPI_INSTANCE]->intr_en = SPI_RX_HALF_INTR_EN;
