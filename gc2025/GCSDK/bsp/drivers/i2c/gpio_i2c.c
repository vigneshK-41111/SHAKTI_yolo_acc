...
#else
	gpiov2_instance->direction = gpiov2_instance->direction | I2C_SCL;
#endif
	log_debug("SCL set as output\n");
}
...
static void SetSdaDirection(int inOutCntrl)
{
	...
	if (inOutCntrl) 
	{ // output
		gpiov2_instance->direction = gpiov2_instance->direction | I2C_SDA;
		log_debug("\n\tSDA set as output\n");
	}
    else 
	{
...
#else
//	sda=1;
	gpiov2_instance->set = I2C_SCL | I2C_SDA;
	delay_loop(delay, delay);

//		scl=1;
	gpiov2_instance->set = I2C_SCL;
	delay_loop(delay, delay);

//		sda=0;
	gpiov2_instance->clear = I2C_SDA;
	delay_loop(delay, delay);

//		scl=0;
	gpiov2_instance->clear = I2C_SCL;
	delay_loop(delay, delay);
#endif
...
//		d1 = sda;
		readGpioData = gpiov2_instance->daa  & I2C_SDA;
		if (readGpioData != 0)
			bitValue = 1;
		else
			bitValue = 0;
...
//	scl=1;delay
	...
	delay_loop(delay, delay);
		...
		log_debug("\n\tI2C: I2C Write\n");
		readData = gpiov2_instance->data  & I2C_SDA;
...
void I2cInit()
{
  SetSCLAsOutput();
  SetSdaDirection(GPIOD_IS_OUT);
  log_debug("Initialization successfully done");
}
...
//This part may not be needed. Need to verify once.
  //	sda=1;
#ifndef YAMUNA || GANGA
  write_word(GPIO_DATA_REG, (read_word(GPIO_DATA_REG) | I2C_SDA) );
  delay_loop(delay, delay);
...
{
	log_debug("start reading the slave device\n");
	int i = 0;
#ifdef PCF8584
	for (i = 0; i <= count; i++) {
		if (i != 0 && i < count) {
