...
int flash_read(int address)
{
	...
	sspi_enable_txrx(sspi_instance[SSPI_INSTANCE], DISABLE);
#endif
	printf("\nAddress:  %x-%x-%x-%x", address >> 24, address >> 16, address >> 8, address >> 0);
	sspi_instance[SSPI_INSTANCE]->data_tx = 0x0b; //FLASH_FAST_READ_COMMAND;	//Do not shift  command when passing as argument.
	sspi_instance[SSPI_INSTANCE]->data_tx =(uint8_t) ( (address >> 24) & 0xFF) ;	//MSB byte of address.
	sspi_instance[SSPI_INSTANCE]->data_tx =(uint8_t) ( (address >> 16) & 0xFF);	//2nd byte of address.
	sspi_instance[SSPI_INSTANCE]->data_tx =(uint8_t) ( (address >> 8) & 0xFF);	//3rd byte of address.
	sspi_instance[SSPI_INSTANCE]->data_tx =(uint8_t) ( (address >> 0) & 0xFF);	//4th byte of address.
	...
	sspi_configure_comm_mode(sspi_instance[SSPI_INSTANCE], HALF_DUPLEX);
	printf("\n Fifo count: %x", sspi_instance[SSPI_INSTANCE]->fifo_count );
	sspi_enable_txrx(sspi_instance[SSPI_INSTANCE], ENABLE);
#ifdef SHOW_REG
	sspi_read_registers(sspi_instance[SSPI_INSTANCE]);
#endif	
	...
	while(1)
	{
		if((sspi_instance[SSPI_INSTANCE]->fifo_count & 0xff)  != 0)
			printf("\n Count: %x; Data: %x", sspi_instance[SSPI_INSTANCE]->fifo_count, sspi_instance[SSPI_INSTANCE]->data_rx );
		else 
			break;
	}
	return recv_data;
...
int flash_read(int address)
{
	...
	read_data[2] = sspi_instance[SSPI_INSTANCE]->data_rx;
	read_data[3] = sspi_instance[SSPI_INSTANCE]->data_rx;
	recv_data = ( (read_data[0] << 24) | (read_data[1] << 16) | (read_data[2] << 8) | (read_data[3] << 0) );	printf("\n d[0]: %x; d[1]: %x;  d[2]: %x;  d[4]: %x; recv_data: %x", read_data[0], read_data[1], read_data[2], read_data[3], recv_data) ;
	return recv_data;
}
#endif
